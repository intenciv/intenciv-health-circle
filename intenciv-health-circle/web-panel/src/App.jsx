...
function RequireAdmin({ children }) {
  ...
  if (!user || !tokens.getAccess() || user.role !== 'admin') return <Navigate to="/login" replace />;
  return children;
}

function RequireReception({ children }) {
  const user = tokens.getUser();
  if (!user || !tokens.getAccess() || !['reception', 'admin'].includes(user.role)) return <Navigate to="/login" replace />;
  return children;
}

function RequireSalesperson({ children }) {
  const user = tokens.getUser();
  if (!user || !tokens.getAccess() || user.role !== 'salesperson') return <Navigate to="/login" replace />;
  return children;
}
