...
export default function Salespersons() {
  ...
  const [open, setOpen]     = useState(null); // null | 'new' | { id, …editing }
  ...
  return (
    <div className="col" style={{ gap: 18 }}>
      <div className="between">
        <h1>Salespersons</h1>
        <button onClick={() => setOpen('new')}>+ New salesperson</button>
      </div>
      {err && <div className="error-banner">{err}</div>}

      <table>
        <thead><tr><th>Name</th><th>Phone</th><th>Today</th><th>Week</th><th>Month</th><th>Total</th><th>Last login</th><th>Status</th><th></th></tr></thead>
        <tbody>
          {list.map(s => (
            <tr key={s.id}>
              <td>{s.full_name}</td>
          ...
          {list.length === 0 && <tr><td colSpan={9} style={{ textAlign: 'center', color: 'var(--text-mid)' }}>No salespersons yet.</td></tr>}
        </tbody>
      </table>

      {open === 'new' && (
        <Dialog title="Add salesperson" onClose={() => setOpen(null)} onSave={create} saving={saving} disabled={!draft.full_name || !draft.phone || !/^\d{4}$/.test(draft.pin)}>
      ...
      {pinOpen && (
        <Dialog title={`Reset PIN for ${pinOpen.full_name}`} onClose={() => { setPinOpen(null); setNewPin(''); }} onSave={resetPin} disabled={!/^\d{4}$/.test(newPin)}>
          <p style={{ color: 'var(--text-mid)', marginBottom: 12 }}>Share the new PIN with the salesperson verbally. They will use it to sign in and to authorize each card activation.</p>
